 ...
 * 2017.02.08
 */
function SqlStr($TableName="",$Coll="",$WhereStr="",$WhereStr2="",$PageIndex="",$PageSize="",$OrderKey="",$OrderType="",$join="",$joinId="",$min="",$max=""){
	$sql = "";
	if ($WhereStr == ""){
		$WhereStr = "Where 1=1";
		$WhereStr2 = "";
	...
	//$sql = "select ".$Coll." from " . $TableName . " ".$join." " . $WhereStr2 . "  AND  $joinId > '".$min."' AND $joinId < '".$max."' order by " . $OrderKey . " " . $OrderType." "."limit ".$PageSize;
	if ($PageIndex > 1){
		...
		//$sql = "select ".$Coll." from " . $TableName . " ".$join." " . $WhereStr2 . "  AND  $joinId > '".$min."' AND  $joinId < '".$max."' order by " . $OrderKey . " " . $OrderType." ";
 ...
 * 公共分页sql2
 */
function SqlStr1($TableName="",$Coll="",$WhereStr="",$WhereStr2="",$PageIndex="",$PageSize="",$OrderKey="",$OrderType="",$join="",$joinId="",$min="",$max=""){
	$sql = "";
	if ($WhereStr == ""){
		$WhereStr = "Where 1=1";
		$WhereStr2 = "";
 ...
 * 晓磊升级版
 */
function SqlStr2($TableName="",$Coll="",$WhereStr="",$WhereStr2="",$PageIndex="",$PageSize="",$OrderKey="",$OrderType="",$join="",$joinId="",$group="",$str=""){
 ...
 * 轮播图专用sql
 */
function SqlStrImg($TableName="",$Coll="",$WhereStr="",$WhereStr2="",$PageIndex="",$PageSize="",$OrderKey="",$OrderType="",$join="",$joinId="",$group=""){
    $min 	= ($PageIndex - 1) * $PageSize;
    $max 	= $PageSize;
    $sql = "SELECT
    {$Coll}
    ...
    {$group}	ORDER BY
    {$OrderKey} {$OrderType}  LIMIT {$min},{$max}
    ";
    return $sql;
}
/**
 ...
 * 晓磊升级版
 */
function SqlStr3($TableName="",$Coll="",$WhereStr="",$WhereStr2="",$PageIndex="",$PageSize="",$OrderKey="",$OrderType="",$join="",$joinId=""){
	$min 	= ($PageIndex - 1) * $PageSize;
	$max 	= $PageSize;
	$WhereStr .=$WhereStr;
	$sql = "SELECT
 ...
 * 2017.02.27
 */
function PageText($url="",$page="",$pagenum="",$show=""){
 ...
 *2017.03.10
 */
function PageTextJS($url="",$page="",$pagenum="",$show=""){
	// 		$html = "<span class='disabled'>«上一页</span>";
	// 		$html .="<span class='current'>1</span>";
	// 		$html .="<a href=\"\">..</a>";
	// 		$html .="<a href=\"\">下一页»</a>";
	...
	if($page == 1){
		...
		$html .="<a href=\"javascript:void(0)\" class='page' rel=".$url.$spage.">«上一页</a>";
	}
	if($page > ($show + 1)){
		$html .= "<a href=\"javascript:void(0)\" class='page' rel=".$url.($page - $show - 1).">..</a>";
	}
	for($i=$page-$show;$i<=$page+$show;$i++){
		if($i == $page){
			$html .= "<span class='current'>".$page."</span>";
		}else if($i>0 && $i<=$pagenum){
			$html .="<a href=\"javascript:void(0)\" class='page' rel=".$url.$i.">".$i."</a>";
		}
	}
	if($page < ($pagenum - $show)){
		$html .="<a href=\"javascript:void(0)\" class='page' rel=".$url.($page + $show + 1).">..</a>";
	...
	if($page == $pagenum){
		...
		$html.="<a href=\"javascript:void(0)\" class='page' rel=".$url.$xpage.">下一页»</a>";
	}
	return $html;
}
 ...
 * @joinkey 数组中的键名
 */
function join_carouse_url($array,$url,$joinkey){
 ...
 * 2017.02.24
 */
function json_array_cart($json){
	if(is_object($json)){
		$json = (array)$json;
		foreach ($json as $key){
			if(is_object($key)){
				$key 	= json_array_cart($json);
			}
		}
	}
	return $json;
 ...
 * 2017.02.18
 */
function returnArrayBool($result){
	if(is_bool($result)){
		return false;
	}else{
		if(!empty($result)){
 ...
 * @return string|number
 */
function CountNumber($array,$key){
	if(empty($array)){
		return "0";
	}
	$total = 0;
 ...
 * @return mixed
 */
function postCurl1($url,$post_data,$body, $header){
...
/**
 * 判断某一天是否为休息日 默认不传参数返回的是当天的信息
 * @param string $time
 * @param number $sate
 * @return unknown|string
 */
function wpjam_is_holiday($time=""){
	$time 		= $time?$time:date("Ymd");
...
function Rewrite_log($filename,$str){
...
function array_unset($arr,$key){   //$arr->传入数组   $key->判断的key值
	...
	if(!empty($res)){$res = array_merge(array(),$res);}
	return $res;
}
//设置返回数组
function reTurnJSONArray($code,$text){
    return array("code"=>$code,"message"=>$text);
}
//组合一维数组
//组合一维数组
function unlimitedForLevel1($cate, $html = '&nbsp;&nbsp;&nbsp;', $pid = 0, $level = 0) {
	$arr = array();
	foreach ($cate as $k => $v) {
		if ($v['parentid'] == $pid) {
			$v['level'] = $level + 1;
			...
			$arr[] = $v;
			$arr = array_merge($arr, unlimitedForLevel1($cate, $html, $v['classid'], $level + 1));
		}
	}
	return $arr;
}
