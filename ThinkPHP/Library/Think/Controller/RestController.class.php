...
class RestController extends Controller {
   ...
   /**
     ...
     * @return void
     */
    protected function response($data,$callback="") {
        //echo ($callback."(".json_encode($data).")");die;
        if(!empty($callback)){
           echo ($callback."(".json_encode($data).")");
           exit();
        }else{
           exit($this->encodeData($data,'json'));
        }
    ...
    protected function Online(){
        $filename = './online.json';  //数据文件，如果没有新建
        $cookiename = 'VGOTCN_OnLineCount';  //session名称
        ...
        */
        ...
        @ 创建访问者通信状态
        使用session通信
        session 将在关闭浏览器时失效，但如果不关闭浏览器，此 session 将一直有效，直到程序设置的在线时间超时
        */
        if(isset($_SESSION[$cookiename])) {  //如果有COOKIE即并非初次访问则不添加人数并更新通信时间
            $uid = $_SESSION[$cookiename];
