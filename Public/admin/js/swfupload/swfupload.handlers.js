...
function InitSWFUpload(upurl, uppath, upsize, flashurl, iswater, isthumbnail) {
    var sendUrl = upurl + "?type=uploadpic&UpFilePath=" + uppath;
...
function del_img(obj) {
    ...
    node.remove(); //删除DOM元素
    //检查是否为封面
    if (focusphoto.val() == smallimg.replace('rect','S')) {
        focusphoto.val("");
        if ($("#show_list ul li .img_box img").length > 0) {
            var firtimg = $("#show_list ul li .img_box img").eq(0);
            firtimg.parent().addClass("current"); //取第一张做为封面
            focusphoto.val(firtimg.attr("src").replace('rect', 'S')); //重新给封面的隐藏域赋值
        }
    }
}
...
function focus_img(obj) {
    $("#focus_photo").val($(obj).children("img").eq(0).attr("src").replace("rect","S"));
...
function show_remark(obj) {
    ...
    var html = "<div style='padding:10px;'><textarea id='ImageRemark' name='ImageRemark' style='font-size:12px;padding:3px;color:#000;border:1px #d2d2d2 solid;vertical-align:middle;width:300px;height:50px;'>" + $(hidRemark).val() + "</textarea></div>";
    var submit = function (v, h, f) {
        if (f.ImageRemark == '') {
            $.jBox.tip("总该写点吧？", 'error', { focusId: "ImageRemark" }); 
            return false;
        }
        $(hidRemark).val(f.ImageRemark);
        $(obj).prevAll(".img_box").find(".remark i").html(f.ImageRemark);
        return true;
    };

    $.jBox(html, { title: "请输入描述？", submit: submit }); 
}
